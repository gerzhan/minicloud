 ...
 * @api private
 */
var recursiveCreateFolder = function*(userId, parentFile, subFolders, options) {
        ...
        return yield recursiveCreateFolder(userId, childFolderFile, nextSubFolders, options)
    }
    /**
     * Create folder 
     * @param {Object} device 
     ...
     * @api private
     */
var createFolder = function*(device, filePath, options) {
    options = options || {}
    ...
    if (!file) {
        ...
        file = yield recursiveCreateFolder(userId, null, subFolders, options)
    } else {
...
var createFile = function*(device, filePath, version, options) {
    ...
    var parentFolder = yield createFolder(device, parentPath, options)
    if (parentFolder) {
...
var getFullFile = function*(file) {
     ...
     * @api private
     */
var fakeDeleteFile = function*(device, filePath, options) {
    options = options || {}
    options.ip = options.ip || '127.0.0.1'
    options.device = device
    var userId = device.user_id
