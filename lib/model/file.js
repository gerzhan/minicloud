...
var MiniCopyFile = function(device, fromPath, toPath, options) {
     ...
     * @api private
     */
MiniCopyFile.prototype._copyFolder = function*(fromFile) {
        var userId = this.device.user_id
        var folderPath = fromFile.path_lower
        var files = yield getDescendantsByPath(userId, folderPath)
        var rootPath = fromFile.path_lower
        for (var i = 0; i < files.length; i++) {
            var df = files[i]
            var name = df.name
            var lowerPath = df.path_lower
            var relativePath = lowerPath.subString(rootPath.length, lowerPath.length)
            var toPath = path.join(toPath, relativePath)
            yield this._copyOneFile(df)
        }
    }
    /**
     * Copy file or folder 
...
MiniCopyFile.prototype.copy = function*() {
        ...
        fromPath = fileHelpers.normalizePath(fromPath)
        ...
        if (fromFile.type === 1) {
            //copy folder
            yield this._copyFolder(fromFile)
        }
    }
    /*
     ...
     * @api private
     */
MiniCopyFile.prototype._copyFile = function*(fromFile) {
        var device = this.device
        var userId = device.user_id
        var toPath = this.toPath
        toPath = fileHelpers.normalizePath(toPath)
        ...
        if (toFile) {
            ...
            toName = path.basename(toPath)
        }
        //create to file parent file
        var toFileParentPath = path.dirname(toPath)
        ...
        if (fromFile.type === 0) {
            ...
            yield fileModel.create({
                ...
                path_lower: fileHelpers.lowerPath(userId, toPath),
                is_deleted: 0,
                mime: fileHelpers.getMime(toPath)
            }, {
                device: this.device,
                version: version
            })
