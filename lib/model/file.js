...
fileModel.hook('beforeUpdate', function(file, options) {
     ...
     * @api public
     */
exports.createFile = function*(userId, filePath, version,options) {
    ...
    if (file) {
        ...
        if (file.version_id == version.id) {
            ...
            //update file
            //TODO add file "document.txt" might become "document (2).txt"
            //TODO overwrite file history versions
            //TODO update "document.txt" might become "document (conflicted copy).txt" or "document (Panda's conflicted copy).txt".
        }
    }
    //create parent file
    var parentId = -1
