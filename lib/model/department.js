'use strict'
/**
 * database table miniyun_groups CRUD
 */
var departmentModel = dbPool.departmentModel

/**
 * Create department
 * @param {Integer} parentId
 * @param {String} name
 * @return {Object}    
 * @api public
 */
exports.create = function*(parentId, name) {
    var department = yield departmentModel.coCreate({
        user_id: -1,
        name: name,
        description: '',
        parent_id: parentId

    })
    return department
}

/**
 * Remove department
 * @param {Integer} id
 * @api public
 */
exports.remove = function*(id) {
    var condition = {
        id: id
    }
    var count = yield departmentModel.coCount(condition)
    if (count > 0) {
        yield departmentModel.find(condition).remove().coRun()
    }
}

/**
 * Rename department
 * @param {Integer} id
 * @param {String} newName
 * @api public
 */
exports.rename = function*(id, newName) {
    var deparment = yield departmentModel.coFind({
        id: id
    })
    var deparment = deparment[0]
    deparment.name = newName;
    yield deparment.coSave()
}

/**
 * exist department or not
 * @param {Integer} parentId
 * @param {String} name
 * @return {Boolean}  
 * @api public
 */
exports.exist = function*(parentId, name) {
    var departmentList = yield departmentModel.coFind({
        parent_id: parentId,
        name: name
    })
    return departmentList.length > 0
}

/**
 * find department by parentId
 * @param {Integer} parentId 
 * @return [Array]
 * @api public
 */
exports.getChildren = function*(parentId) {
    var departmentList = yield departmentModel.coFind({
        parent_id: parentId
    })
    return departmentList

}

/**
 * judge department exit or not by id
 * @param {Integer} id
 * @return {Boolean}  
 * @api public
 */
exports.getById = function*(id) {
    var departmentList = yield departmentModel.coFind({
        id: id
    })
    if (departmentList.length > 0) {
        return departmentList[0]
    }
    return null
}

/**
 * exist department or not
 * @param {Integer} parentId
 * @param {String} name
 * @return {Boolean}  
 * @api public
 */
exports.getDepartment = function*(parentId, name) {
    var departmentList = yield departmentModel.coFind({
        parent_group_id: parentId,
        name: name
    })
    return departmentList
}
