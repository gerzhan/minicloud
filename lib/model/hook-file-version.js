...
var versionModel = sequelizePool.versionModel
     ...
     * @api private
     */
var createFileVersionMeta = function*(version, options) {
    options = options || {}
    ...
    if (session) {
        ...
        //create origin meta
        var user = yield MiniUser.getById(options.device.user_id)
        var filePath = PATH_SEP+path.join(user.name,options.path) 
        yield MiniVersionMeta.create(version.id, 'path', filePath)
    }
}
...
var hook = function() {
    versionModel.hook('afterCreate', function(version, options) {
        return co.wrap(function*() {
            yield createFileVersionMeta(version, options.context)
        })()
    })
}
exports.run = hook
