...
exports.search = function*(key) {
    var orm = require('orm')
    var aimIds = {}
    var userList = yield userModel.find({user_name: orm.like('%'+key+'%')}).coRun()
    for(var i=0;i<userList.length;i++){
        aimIds[userList[i].id] = userList[i].id
    }
    var userMetaList = yield userMetaModel.find({meta_key:"nick"}).find({meta_value: orm.like('%'+key+'%')}).coRun()
    ...
    var result = yield userModel.find({id:idsInArray}).coRun()
    return result
}
