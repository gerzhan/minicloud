...
exports.resetPasswd = function*(userId, oldPasswd, newPasswd) {
    ...
    /**
     * Judge whether it is the last super administrator
     * @param {Object} user 
     * @return {Boolean}   
     * @api public
     */
var isLastSuperAdmin = function*(user) {
        var admins = yield userModel.findAll({
            where: {
                status: 1,
                role: SUPER_ADMIN
            }
        ...
        var result = false
        if (admins.length === 1) {
            var lastAdmin = admins[0]
            if (lastAdmin.name === user.name) {
                result = true
            }
        }
        return result
    }
exports.isLastSuperAdmin = isLastSuperAdmin     
    /**
     * set user status
     * @param {Object} user
     ...
     * @api public
     */
exports.setStatus = function*(user, isEnabled) {
        if (user) {
            if (isEnabled === 0) {
                //last super admin don't disabled
                if (yield isLastSuperAdmin(user)) {
                    return user
                }
            }
     ...
     * @api public
     */
exports.setRole = function*(user, role) {
        if (role !== SUPER_ADMIN) {
            //last super admin don't change role
            if (yield isLastSuperAdmin(user)) {
                return user
            }
        }
