...
exports.getPageBootrap = function*(app) {
        ...
        content = S(content).replaceAll('@static_host',appContext.static_host) 
        content = S(content).replaceAll('@sub_app_name','login')
        content = S(content).replaceAll('@sub_app_bname','Login')
        content = S(content).replaceAll('@version',appVersion) 
