...
var MiniRefreshToken = require("../../../model/oauth-refresh-token")
var MiniClient = require("../../../model/mini-client")
 ...
 * According bearerToken,return accessToken
...
model.getAccessToken = function(bearerToken, callback) {
    ...
    /**
     * koa-oauth-server's function.
     * According bearerToken,return refreshToken
     * @param {String} bearerToken
     * @param {Function} callback
     * @api public
     */
...
model.getClient = function(clientId, clientSecret, callback) {
        co.wrap(function*() {
            var app = yield MiniClient.getApp(clientId, clientSecret)
            if (app) {
...
model.saveAccessToken = function(accessToken, clientId, expires, user, callback) {
        co.wrap(function*() {
            yield MiniAccessToken.create(user.id, clientId, accessToken, expires.getTime())
            callback(false);
        })()
    }
    /**
...
model.saveRefreshToken = function(refreshToken, clientId, expires, user, callback) {
        co.wrap(function*() {
            yield MiniAccessToken.create(user.id, clientId, refreshToken, expires.getTime())
            callback(false);
        })()
    }
    /**
...
model.getUser = function(username, password, bodyContext, callback) {
    co.wrap(function*() {
        ...
        yield MiniUserMeta.resetPasswordErrorTimes(user.id)
            //create user device
        yield MiniUserDevice.create(user, bodyContext.device_name, bodyContext.app_key)
