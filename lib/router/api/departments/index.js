...
exports.membersAdd = function*() {
    ...
    if (!department) {
        ...
        if (member == null) {
            ...
            yield MiniUserDepartmentRelation.create(id, member.id, department.path)
            this.body = ''
        }
    }
}
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        for (var m = 0; m < departArr.length; m++) {
            if (m == 0) {
                ...
                var department = yield MiniDepartment.getDepartment(departmentId, departArr[m])
                if (!department) {
                    if (departArr[m] != '') { //avoid null name
                        var depart = yield MiniDepartment.create(departmentId, departArr[m])
                        departmentId = depart.id
                        departPath = depart.path
                    }
                } else {
            ...
            if (m == departArr.length - 1) { //start to import users
                for (var n = 0; n < memberArr.length; n++) {
                    ...
                    if (userExist) {
                        ...
                        yield MiniUserDepartmentRelation.create(departmentId, user.id, departPath)
                        successCount++
