...
function extractCredentials (done) {
  ...
  // 强制由application/x-www-form-urlencoded修改为application/json
  if (this.req.method !== 'POST' ||
      !this.req.is('application/json')) {
    return done(error('invalid_request',
      'Method must be POST with application/json encoding'));
  } 
  // Grant type
  this.grantType = this.req.body && this.req.body.grant_type;
  // grantType为空，默认值设置为password
  if(!this.grantType){
...
function credsFromBody (req) {
  ...
  if(!clientId){
    //clientId设置为app_key，不是此前的client_id
    clientId = req.body.app_key
  }
  var clientSecret = req.body.client_secret
  if(!clientSecret){
    //clientSecret设置为app_secret，不是此前的client_secret
    clientSecret = req.body.app_secret
  } 
...
function usePasswordGrant (done) {
  ...
  //用户名设置由此前的username修改为user_name
  if(!uname){
    uname = this.req.body.name
  }
  if (!uname || !pword) {
    return done(error('invalid_client',
      'Missing parameters. "name" and "password" are required'));
  }

  var self = this;
  //把上下文传递到model中
  return this.model.getUser(uname, pword,this.req.body, function (err, user) {
    //账号与密码验证还有多种可能，这里设置为invalid_grant
    if (err) return done(error('invalid_grant', false, err));
    if (!user) {
