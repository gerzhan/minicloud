...
describe(protocol + ' departments users add', function() {
    ...
    it(protocol + ' departments/users/add 200', function*(done) {
        var res = yield request(app)
            .post('/api/v1/departments/users/add')
            .type('json')
            .set({
            ...
            user.department_path.should.equal(department.path)
        done()
    })
    ...
    it(protocol + ' departments/users/add 200', function*(done) {
        var res = yield request(app)
            ...
            .expect(200)
            .end()
            var user = yield MiniUser.getByUuid(uuid2)
            user.department_path.should.equal(department2.path)
        done()
    })
    ...
    it(protocol + ' departments/users/add 200', function*(done) {
        var res = yield request(app)
            ...
            .expect(200)
            .end()
            var user = yield MiniUser.getByUuid(uuid3)
            user.department_path.should.equal(department3.path)
        done()
    })
    it(protocol + ' departments/users/add socket.io  200', function*(done) {
    ...
    it(protocol + ' departments/users/add 400', function*(done) {
        var res = yield request(app)
            .post('/api/v1/departments/users/add')
            .type('json')
            .set({
    ...
    it(protocol + ' departments/users/add 401', function*(done) {
        var res = yield request(app)
            .post('/api/v1/departments/users/add')
            .type('json')
            .set({
    ...
    it(protocol + ' departments/users/add 401 require_administrator_token', function*(done) {
        var res = yield request(app)
            .post('/api/v1/departments/users/add')
            .type('json')
            .set({
    ...
    it(protocol + ' departments/users/add 409 department_not_exist', function*(done) {
        var res = yield request(app)
            .post('/api/v1/departments/users/add')
            .type('json')
            .set({
    ...
    it(protocol + ' departments/users/add 409 user_not_exist', function*(done) {
        var res = yield request(app)
            .post('/api/v1/departments/users/add')
            .type('json')
            .set({
